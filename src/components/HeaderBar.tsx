...
export function HeaderBar({ repoName, currentBranch, isDirty, runningCount, readyToMergeCount }: HeaderBarProps) {
  return (
    <box style={{ paddingLeft: 1, paddingRight: 1, paddingTop: 1, paddingBottom: 1, backgroundColor: "#222222", flexDirection: "row", justifyContent: "space-between", height: 3 }}>
      ...
      <box style={{ flexDirection: "row", gap: 1 }}>
        ...
        {runningCount > 0 && readyToMergeCount > 0 && (
          <text fg="#555555">{"•"}</text>
        )}
        {readyToMergeCount > 0 && (
          <text fg="#ff9900">{"↑"} {readyToMergeCount}</text>
        )}
      </box>
    </box>
  )
